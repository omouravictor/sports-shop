...
public class ShirtCRUD extends AbstractJDialog<Shirt> {
    ...
    private TbShirtModel tbShirtModel;
    ...
    public ShirtCRUD(java.awt.Frame parent, boolean modal, ShirtManager shirtManager, TbShirtModel model) {
        super(parent, modal);
        this.shirtManager = shirtManager;
        this.tbShirtModel = model;
        initComponents();
    }
    ...
    private void btCreateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btCreateActionPerformed
        try {
            Shirt newShirt = shirtManager.create();
            tbShirtModel.addObjectRow(newShirt);
        } catch (Exception ex) {
            showErrorMessage(ex.getMessage());
        }
        tbShirt.clearSelection();
    ...
    private void btReadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btReadActionPerformed
        if (rowIsSelected(tbShirt)) {
            try {
                ...
                shirtManager.read(shirtSelected);
            } catch (Exception ex) {
                showErrorMessage(ex.getMessage());
            }
            tbShirt.clearSelection();
    ...
    private void btUpdateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btUpdateActionPerformed
        if (rowIsSelected(tbShirt)) {
            Shirt shirtSelected = getObjectSelectedInTb(tbShirt, tbShirtModel);
            Shirt shirtOriginal = new Shirt(shirtSelected);
            try {
                Shirt updatedShirt = shirtManager.update(shirtSelected);
                tbShirtModel.updateObjectRow(shirtSelected, updatedShirt);
            } catch (Exception ex) {
                showErrorMessage(ex.getMessage());
                tbShirtModel.updateObjectRow(shirtSelected, shirtOriginal);
            }
            tbShirt.clearSelection();
        }
    ...
    private void btDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btDeleteActionPerformed
        if (rowIsSelected(tbShirt)) {
            try {
                ...
                if (answer == 0) {
                    shirtManager.delete(shirtSelected);
                    tbShirtModel.removeObjectRow(tbShirt.getSelectedRow());
                }
            } catch (Exception ex) {
                showErrorMessage(ex.getMessage());
            }
